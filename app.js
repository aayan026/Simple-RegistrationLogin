...
let filteredArray = [...new Set(goods)]

while(true){
  let choice=prompt("1.Find object\n2.Filter price")
  if(choice=='1'){
    let name=prompt("Enter name")
     let find=filteredArray.find(item=>item.product_name===name)
     if(find){
        alert(
         `Product name: ${find.product_name}\n` +
          `Price: ${find.product_price}\n` +
          `Description: ${find.product_description}\n` +
          `Store name: ${find.store_name}\n` +
          `Store address: ${find.store_address}\n` +
          `Id: ${find.id}\n` +
          `------------------------------------------------------------`  );  }
      else{
    alert("not found")
    ...
    let min=Number(prompt("Enter minimum price"))
    let max=Number(prompt("Enter maximum price"))

    let result=filteredArray.filter(item=>item.product_price>=min && item.product_price<=max)
    if (result.length > 0) {
      alert(result.map(item =>
    ...
    alert("No products found in this price range!");
  }
  }
}